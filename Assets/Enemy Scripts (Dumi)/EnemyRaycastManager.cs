...
public class EnemyRaycastManager : MonoBehaviour
{
    #region PUBLIC FIELDS: // using hashtag region to organize  the code better into sections
    [Header("Layer Masks")]
    public LayerMask rayMask;
    public bool debuggingRays;

    public float rayDistance; // distance of the ray casted frim the enemy to exactly where the player is for movement to occur
    ...
    [Header("Middle")]
    [Range(3, 25)]
    public int midPointsX = 3; // // Number of rays in X direction for the middle
    [Range(3, 25)]
    ...
    void Start()
    {
        ...
        if (gameObject.tag == "Player") // when the enemy game object picks up a game object tagged as the player, all comp that relate to the enemy movement are called
        {
            _controller = GetComponent<CharacterController>(); //for the player
    ...
    void FixedUpdate()
    {
        GetPoints(); //calling ray origins method here to calculate the ray origins  of the top, bottom and middle points
        DebuggingRays(); // drawing the draw raw debugs if enabled
    }
    ...
    void GetMidPoints() // this method uses a for loop to calculate the mid ray origins
    {
        int count = 0;
    ...
    void GetTopPoints() // calculating top ray points
    {
        int count = 0;
    ...
    { // this raycast is casted on the bottom point of the player game object to trigger the enemy movement.
    ...
    void DebuggingRays()
    {
        if (debuggingRays) // this is just for debugging purposes to see of the raycasts on different pooint are working 
        {
            int count = 0;
